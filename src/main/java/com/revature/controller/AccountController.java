...
public class AccountController implements Controller{
	...
	private Handler addAccountForClient = ctx -> {
		...
		logger.info("Account: " + "'" + addedAccount.getAccountName() +"'" + " was added successfully for client with id of " + clientId);
	...
	private Handler updateAnAccountByIdForClient = ctx -> {
		...
		logger.info("Account with id of " + account.getId() + " was updated in the database for client with id of " + clientId);
		ctx.json(account);
		ctx.status(201);
	...
	private Handler deleteAnAccountForClient = ctx -> {
		...
		logger.info("Account with id of " + accountId + " was deleted in the database for client with id of " + clientId);
		ctx.result("Account with id of " + accountId + " was deleted in the database for client with id of " + clientId);
		ctx.status(201);
	};
